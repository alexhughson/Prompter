from prompter.schemas import Prompt, UserMessage, ImageMessage
...
def test_image_with_question(llm_executor):
    ...
    prompt = Prompt(
        ...
        messages=[
            ImageMessage(url=CAT_IMAGE),
            UserMessage(content="What color is this animal?"),
        ],
    )
